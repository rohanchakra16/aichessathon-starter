...
from training.train_stockfish_evaluator import (
    FEATURES,
    coefficient_prior,
    features,
    generated_positions,
...
def test_generated_positions_are_deterministic_and_nonterminal() -> None:
    ...
    assert all(is_quiet(board) for board in first)
...
def test_material_prior_keeps_conventional_piece_scale() -> None:
    prior = coefficient_prior()
    assert prior[1] == 100.0
    assert prior[1 + 64] == 320.0
    assert prior[1 + 4 * 64] == 900.0
