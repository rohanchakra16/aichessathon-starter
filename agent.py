...
PASSED_MASKS = tuple(
    tuple(_passed_mask(colour, square) for square in chess.SQUARES)
    for colour in chess.COLORS
)


def _strategic_values(board: chess.Board, colour: chess.Color) -> tuple[float, ...]:
    """Return cheap relational features that a piece-square table cannot encode."""
...
def _model_evaluate(board: chess.Board) -> float:
    ...
    if board.has_queenside_castling_rights(not side):
        score -= WEIGHTS[CASTLING_OFFSET + 1]
    for index, value in enumerate(_strategic_difference(board)):
        score += value * (
            blend * WEIGHTS[STRATEGIC_OFFSET + index]
            + (1.0 - blend) * WEIGHTS[STRATEGIC_OFFSET + STRATEGIC_FEATURES + index]
        )
    return BIAS + score
